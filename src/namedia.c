...
CURLcode execute_query(CURL *curl, char *url, Response *response_ptr) {
  ...
  curl_easy_setopt(curl, CURLOPT_WRITEFUNCTION,
                   curl_callback); // set callback function
  curl_easy_setopt(curl, CURLOPT_WRITEDATA,
                   (void *)response_ptr); // set the pointer to response struct
...
int search(CURL *curl, char query[]) {
  ...
  json_object *request_json =
      json_object_from_file("data/youtube-search-request.json");
  ...
  if (result_code != CURLE_OK || response_ptr->size < 1) {
    fprintf(stderr, "ERROR: Failed to fetch url (%s) - curl failed with: %s",
            url, curl_easy_strerror(result_code));
    return 2;
  }
  ...
  printf("Parsed JSON: \n%s\n", json_object_to_json_string_ext(
                                    response_json, JSON_C_TO_STRING_PRETTY));
  json_object_put(response_json);
  return 0;
}
